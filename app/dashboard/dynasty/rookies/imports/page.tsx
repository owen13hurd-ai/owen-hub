...
export default async function RookieImportsPage() {
  ...
  return <div className="space-y-6"><Link href="/dashboard/dynasty/rookies" className="inline-flex items-center gap-2 text-sm font-semibold text-ink/60 hover:text-moss"><ArrowLeft className="h-4 w-4" />Rookie rankings</Link><PageHeader eyebrow="Rookie imports" title="Duplicate resolution" description="Review possible name matches explicitly. The engine never performs a fuzzy merge automatically." /><RookieDuplicateQueueClient duplicates={duplicates} readyBatches={readyBatches} /></div>;
}
