...
import { importCfbdRookieSeasons, importOpenRookieEnrichments } from "@/app/dashboard/dynasty/rookies/actions";
...
export function RookiePublicDataImportClient() {
  ...
  return <section className="rounded-lg border border-ink/10 bg-white p-5 shadow-soft">
    ...
    <div className="mt-5 flex flex-col justify-between gap-4 border-t border-ink/10 pt-5 sm:flex-row sm:items-center">
      <div><h3 className="font-bold text-ink">Open-data enrichments</h3><p className="mt-1 max-w-2xl text-sm text-ink/55">Import cfbfastR target share, official NFL early-entry status, and nflverse outcomes from the audited bundled snapshot.</p></div>
      <Button type="button" variant="outline" disabled={enrichmentPending} onClick={() => startEnrichmentTransition(async () => {
        const result = await importOpenRookieEnrichments();
        if (result.ok) toast.success(result.message);
        else toast.error(result.message);
        if (result.ok) router.refresh();
      })}><DatabaseZap className="h-4 w-4" />{enrichmentPending ? "Importing..." : "Import enrichments"}</Button>
    </div>
  </section>;
}
