...
import { commitRookieImport } from "@/app/dashboard/dynasty/rookies/actions";
...
export function RookieDuplicateQueueClient({ duplicates, readyBatches }: { duplicates: RookiePendingDuplicate[]; readyBatches: RookieImportBatchSummary[] }) {
  ...
  function commit(batchId: string) { startTransition(async () => { const result = await commitRookieImport(batchId); if (result.ok) { toast.success(result.message); router.push("/dashboard/dynasty/rookies"); router.refresh(); } else toast.error(result.message); }); }
