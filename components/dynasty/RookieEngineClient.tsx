                ...
                {visibleRankings.map((ranking) => (
                  <tr key={ranking.id} className="border-t border-ink/8 hover:bg-skyglass/35">
                    ...
                    <td className="px-4 py-3">
                      ...
                      {ranking.evidenceFlags.length ? <div className="mt-2 flex max-w-80 flex-wrap gap-1" aria-label={`Evidence flags for ${ranking.name}`}>{ranking.evidenceFlags.map((flag) => <span key={flag.key} title={flag.detail} className={`rounded border px-1.5 py-0.5 text-[9px] font-bold ${flag.tone === "green" ? "border-moss/25 bg-moss/8 text-moss" : "border-ember/25 bg-ember/8 text-ember"}`}>{flag.label}{flag.points === null ? "" : ` ${flag.points > 0 ? "+" : ""}${flag.points}`}</span>)}</div> : null}
